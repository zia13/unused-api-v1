node('windows'){
    stage('create script') {
        ...
        function Get-ApiRequestCount {
            ...
            try {
                ...
                Write-Host "Warning: Failed to get metrics for API $apiId stage $stageName" -ForegroundColor Yellow
                return 0
            }
        }
    ...
    stage('Create Catalog'){
        ...
        for (awsAccountWithName in awsAccountsArr) {
            ...
            withEnv(["AWS_DEFAULT_REGION=${awsDefaultRegion}", "AWS_DEFAULT_OUTPUT=${awsDefaultOutput}"]) {
                withCredentials([[$class: 'StringBinding', credentialsId: "${awsAccountID}-lrdevops-aws-access-key" , variable: 'AWS_ACCESS_KEY_ID'],[$class: 'StringBinding', credentialsId: "${awsAccountID}-lrdevops-aws-secret-key", variable: 'AWS_SECRET_ACCESS_KEY']])
                {
                    echo "cmdResponse :::${awsAccountID} ${awsAccountName}"
                    serialno = powershell(
                        script: """
                        .\\CreateCatalog.ps1 -accountId ${awsAccountID} -accountName "${awsAccountName}" -index ${serialno} -env ${environment}
                        """,
                        returnStdout: true
                    ).trim().toInteger()
                }
